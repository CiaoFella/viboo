...
export function getNextPage() {
  ...
  if (containers.length === 1) {
    ...
    // During transition, the NEXT container has the 'is-animating' class
    const nextContainer = document.querySelector('[data-barba="container"].is-animating')
    const currentContainer = Array.from(containers).find(container => container !== nextContainer)

    // If both containers have the same namespace, prefer the one with 'is-animating' class
    if (
      nextContainer &&
      currentContainer &&
      nextContainer.dataset.barbaNamespace === currentContainer.dataset.barbaNamespace
    ) {
      return nextContainer.dataset.barbaNamespace
    }
...
export function getNextContainer() {
  const containers = document.querySelectorAll('[data-barba="container"]')
  const nextPageName = getNextPage()

  // First try to find by namespace
  let nextContainer = Array.from(containers).find(container => container.dataset.barbaNamespace === nextPageName)

  // If we have multiple containers with same namespace, prefer the one with 'is-animating' class
  if (containers.length === 2 && !nextContainer) {
    const animatingContainer = document.querySelector('[data-barba="container"].is-animating')
    if (animatingContainer) {
      nextContainer = animatingContainer
    }
  }

  return nextContainer || containers[0]
}
